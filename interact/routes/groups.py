...
@login_required
async def api_list_feature_groups(bot_id: int, kind: str):
    from flask import g

    if not await get_bot(g.user["id"], bot_id):
        return jsonify({"items": []})
    groups = await list_feature_groups(bot_id, kind)
    return jsonify({"items": groups})


@api_bp.post("/bots/<int:bot_id>/groups/<string:kind>")
@login_required
async def api_upsert_feature_group(bot_id: int, kind: str):
    from flask import g
    ...
    ok, err, gid = await upsert_feature_group(bot_id, kind, name, description, group_id, active)
...
@login_required
async def api_set_feature_group_active(bot_id: int, group_id: int):
    from flask import g
    ...
    data = request.get_json(silent=True) or {}
    active = data.get("active", True)
    ok, err = await set_feature_group_active(bot_id, group_id, active)
...
@login_required
async def api_delete_feature_group(bot_id: int, group_id: int):
    from flask import g

    if not await get_bot(g.user["id"], bot_id):
        return jsonify({"ok": False, "error": "无权限"}), 403
    ok, err = await delete_feature_group(bot_id, group_id)
    status = 200 if ok else 404
    return jsonify({"ok": ok, "error": err}), status
