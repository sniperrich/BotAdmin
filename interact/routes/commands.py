...
@login_required
def api_set_command_active(bot_id: int, command_id: int):
    from flask import g
    ...
    data = request.get_json(silent=True) or {}
    active = 1 if data.get("active", True) else 0
    ok, err = set_command_active(bot_id, command_id, active)
    status = 200 if ok else 404
    return jsonify({"ok": ok, "error": err}), status
...
@login_required
def api_set_command_group(bot_id: int, command_id: int):
    from flask import g
    ...
    ok, err = set_command_group(bot_id, command_id, group_id)
    status = 200 if ok else 404
    return jsonify({"ok": ok, "error": err}), status
