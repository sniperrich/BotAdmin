...
@login_required
async def api_list_flows(bot_id: int):
    from flask import g

    if not await get_bot(g.user["id"], bot_id):
        return jsonify({"items": []})
    items = await list_flows(bot_id)
    for item in items:
...
@login_required
async def api_upsert_flow(bot_id: int):
    from flask import g
    ...
    ok, err, fid = await upsert_flow(bot_id, name, normalized, flow_id, active, group_id)
...
@login_required
async def api_delete_flow(bot_id: int, flow_id: int):
    from flask import g

    if not await get_bot(g.user["id"], bot_id):
        return jsonify({"ok": False, "error": "无权限"}), 403
    ok, err = await delete_flow(bot_id, flow_id)
...
@login_required
async def api_set_flow_active(bot_id: int, flow_id: int):
    from flask import g
    ...
    ok, err = await set_flow_active(bot_id, flow_id, active)
    status = 200 if ok else 404
    return jsonify({"ok": ok, "error": err}), status
...
@login_required
async def api_set_flow_group(bot_id: int, flow_id: int):
    from flask import g
    ...
    data = request.get_json(silent=True) or {}
    group_id = data.get("group_id")
    ok, err = await set_flow_group(bot_id, flow_id, group_id)
    status = 200 if ok else 404
    return jsonify({"ok": ok, "error": err}), status
