...
@login_required
async def api_list_pseudocode(bot_id: int):
    from flask import g

    if not await get_bot(g.user["id"], bot_id):
        return jsonify({"items": []})
    return jsonify({"items": await list_pseudocode(bot_id)})


@api_bp.post("/bots/<int:bot_id>/pseudocode")
@login_required
async def api_upsert_pseudocode(bot_id: int):
    from flask import g
    ...
    ok, err, pid = await upsert_pseudocode(bot_id, title, content, pseudo_id, active, group_id)
...
@login_required
async def api_delete_pseudocode(bot_id: int, item_id: int):
    from flask import g

    if not await get_bot(g.user["id"], bot_id):
        return jsonify({"ok": False, "error": "无权限"}), 403
    ok, err = await delete_pseudocode(bot_id, item_id)
...
@login_required
async def api_set_pseudocode_active(bot_id: int, item_id: int):
    from flask import g
    ...
    ok, err = await set_pseudocode_active(bot_id, item_id, active)
    status = 200 if ok else 404
    return jsonify({"ok": ok, "error": err}), status
...
@login_required
async def api_set_pseudocode_group(bot_id: int, item_id: int):
    from flask import g
    ...
    data = request.get_json(silent=True) or {}
    group_id = data.get("group_id")
    ok, err = await set_pseudocode_group(bot_id, item_id, group_id)
    status = 200 if ok else 404
    return jsonify({"ok": ok, "error": err}), status
...
@login_required
async def api_generate_flow_from_pseudo(bot_id: int, item_id: int):
    from flask import g

    if not await get_bot(g.user["id"], bot_id):
        return jsonify({"ok": False, "error": "无权限"}), 403
    pseudo = await get_pseudocode(bot_id, item_id)
    if not pseudo:
    ...
    ok, err, fid = await upsert_flow(bot_id, result["name"], compiled_json, None, 1)
    if not ok:
...
@login_required
async def api_sandbox_pseudocode(bot_id: int, item_id: int):
    from flask import g
