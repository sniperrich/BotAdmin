...
@login_required
async def api_list_pro_scripts(bot_id: int):
    from flask import g

    if not await get_bot(g.user["id"], bot_id):
        return jsonify({"ok": False, "error": "无权限", "items": []}), 403
    return jsonify({"ok": True, "items": await list_pro_scripts(bot_id)})


@api_bp.post("/bots/<int:bot_id>/pro_scripts")
@login_required
async def api_upsert_pro_script(bot_id: int):
    from flask import g
    ...
    ok, err, sid = await upsert_pro_script(bot_id, name, command, code, script_id, active, group_id)
...
@login_required
async def api_delete_pro_script(bot_id: int, script_id: int):
    from flask import g

    if not await get_bot(g.user["id"], bot_id):
        return jsonify({"ok": False, "error": "无权限"}), 403
    ok, err = await delete_pro_script(bot_id, script_id)
...
@login_required
async def api_set_pro_script_active(bot_id: int, script_id: int):
    from flask import g
    ...
    ok, err = await set_pro_script_active(bot_id, script_id, active)
    status = 200 if ok else 404
    return jsonify({"ok": ok, "error": err}), status
...
@login_required
async def api_set_pro_script_group(bot_id: int, script_id: int):
    from flask import g
    ...
    data = request.get_json(silent=True) or {}
    group_id = data.get("group_id")
    ok, err = await set_pro_script_group(bot_id, script_id, group_id)
    status = 200 if ok else 404
    return jsonify({"ok": ok, "error": err}), status
